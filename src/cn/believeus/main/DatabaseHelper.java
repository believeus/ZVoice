...
public class DatabaseHelper extends SQLiteOpenHelper// 继承SQLiteOpenHelper类
{
    ...
    // 数据库名
    private static final String DATABASE_NAME = "zvoice.db";

    // 数据表名，一个数据库中可以有多个表（虽然本例中只建立了一个表）
    public static final String TABLE_NAME = "TVideo";
            ...
            int version)
    {
        ...
        DATABASE_VERSION=Integer.parseInt(PropertiesUtil.findValue(context,Variables.DATABASE_VERSION));
    ...
    public void onCreate(SQLiteDatabase db)
    {
        ...
        sql.append("[videoname] TEXT,");
        sql.append("[desc] TEXT,");
        sql.append("[videoUrl] TEXT,");
        sql.append("[localPath] TEXT,");
        sql.append("[username] TEXT,");
        sql.append("[createTime] date,");
        sql.append("[thumb1] TEXT,");
        sql.append("[thumb2] TEXT,");
        sql.append("[thumb3] TEXT,");
        sql.append("[thumb4] TEXT)");
